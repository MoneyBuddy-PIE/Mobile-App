...
export default function ChapterDetail() {
    ...
    const fallbackImage = COURSE_IMAGES[imgIndex % COURSE_IMAGES.length];
    ...
    const [selectedCourse, setSelectedCourse] = useState<CourseWithoutSectionsWithProgress | null>(null);
    ...
    const handleCoursePress = (course: CourseWithoutSectionsWithProgress, index: number) => {
        if (course.locked) {
    ...
    const handleStartCourse = (course: CourseWithoutSectionsWithProgress) => {
    ...
    const renderCourse = (course: CourseWithoutSectionsWithProgress, index: number) => {
        const isCompleted = course.completed;
        const isLocked = course.locked;

        return (
            <View style={styles.courseCont} key={index}>
                {/* Progression indicator */}
                <View style={styles.progressIndicator}>
                    <View style={[styles.progressDot, isCompleted && styles.progressDotCompleted]}>
                        {isCompleted && <Ionicons name="checkmark" size={12} color="#fff" />}
                    </View>
                    {index < (courses.length || 0) - 1 && <View style={[styles.progressLine, isCompleted && styles.progressLineCompleted]} />}
                </View>
                <TouchableOpacity
                        ...
                        <View style={styles.courseHeader}>
                            <View style={styles.courseInfo}>
                                ...
                                <View style={styles.courseMeta}>
                                    ...
                                    <Text style={[styles.courseMetaText, isLocked && styles.courseTextLocked]}>Lecture - {course.readTime} min</Text>
                                </View>
                            </View>
                            ...
                            <View style={[styles.courseImage, isLocked && styles.courseImageLocked]}>
                                {course.imageUrl ? (
                                    <Image
                                        source={{ uri: getImageUrl(course.imageUrl) ?? undefined }}
                                        style={styles.courseImageImg}
                                        resizeMode="cover"
                                    />
                                ) : (
                                    <Ionicons name="book-outline" size={32} color={isLocked ? "#ccc" : "#6C5CE7"} />
                                )}
                            </View>
                        </View>
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.headerContainer}>
                <View style={styles.headerBackground}>
                    {/* Image de fond */}
                    {chapter.imageUrl ? (
                        <Image source={{ uri: getImageUrl(chapter.imageUrl) ?? undefined }} style={styles.headerImage} resizeMode="cover" />
                    ) : (
                        <Image source={fallbackImage} style={styles.headerImage} resizeMode="cover" />
                    )}
                </View>
            ...
            {/* Course Detail Modal */}
            <CourseDetailModal course={selectedCourse} visible={modalVisible} onClose={handleCloseModal} onStartCourse={handleStartCourse} />
        </SafeAreaView>
    );
}
