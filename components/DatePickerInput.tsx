...
import { Modal, View, Text, TouchableOpacity, StyleSheet, ScrollView, NativeScrollEvent, NativeSyntheticEvent } from "react-native";
...
const WheelColumn = memo<WheelColumnProps>(({ items, selectedIndex, onSelect, flex = 1 }) => {
    ...
    const paddedItems = [...Array(PADDING_ITEMS).fill(""), ...items, ...Array(PADDING_ITEMS).fill("")];
                ...
                {paddedItems.map((item, index) => {
                    ...
                    return (
                        <View key={index} style={{ height: ITEM_HEIGHT, justifyContent: "center", alignItems: "center" }}>
                            <Text style={[wheelStyles.text, isSelected && wheelStyles.textSelected]}>{item}</Text>
                        </View>
                    );
                })}
            </ScrollView>
...
const DatePickerInput: React.FC<DatePickerInputProps> = ({ value, onChange, placeholder = "Choisir une date", minYear, maxYear }) => {
    ...
    return (
        <>
            {/* Trigger */}
            <TouchableOpacity onPress={handleOpen} style={[styles.container, isSelected && styles.containerSelected]} activeOpacity={0.7}>
                <Text style={[styles.inputText, !isSelected && styles.placeholder]}>{value ? formatDateFR(value) : placeholder}</Text>
                {isSelected ? (
