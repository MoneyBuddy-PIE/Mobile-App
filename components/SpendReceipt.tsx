...
const ReceiptScallop = ({ width }: { width: number }) => {
    ...
    return (
        <Svg width={totalW} height={SCALLOP_H} style={{ display: "flex" }}>
            <Path d={d} fill="white" />
        </Svg>
    );
};
...
const ReceiptRow = ({ label, value, emoji, isLast = false }: ReceiptRowProps) => (
    <View style={[rowStyles.container, isLast && { borderBottomWidth: 0 }]}>
        ...
        <View style={rowStyles.valueContainer}>
            {emoji &&
                (() => {
                    const Icon = getIconFromCategory(emoji);
                    return (
                        <View style={rowStyles.emojiBadge}>
                            <Icon width={24} height={24} color="#846DED" />
                        </View>
                    );
                })()}
...
const SpendReceipt: React.FC<SpendReceiptProps> = ({ amount, categoryEmoji = "🛒", description, date = new Date(), paymentMethod = "Espèces" }) => {
    const [cardWidth, setCardWidth] = useState(0);

    const parsedAmount = parseFloat(amount);
    const displayAmount = isNaN(parsedAmount) ? "0" : formatMoney(parsedAmount);
    ...
    return (
        <>
            ...
            <View style={styles.receiptWrapper}>
                {/* Carte du reçu */}
                <View style={styles.receiptCard} onLayout={(e) => setCardWidth(e.nativeEvent.layout.width)}>
                    {/* Checkmark vert */}
                    ...
                    <ReceiptRow label="Date" value={formatReceiptDate(date)} />
                    <ReceiptRow label="Catégorie" value={categories.find((cat) => cat.value === categoryEmoji)?.label ?? ""} emoji={categoryEmoji} />
                    <ReceiptRow label="Paiement" value={paymentMethod} isLast={!description} />
