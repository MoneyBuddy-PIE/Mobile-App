...
const ButtonsActionChildren = ({ subAccount, labelPosition = "inside", addScan = false }: IProps) => {
    ...
    const handleButtonClick = (type: string) => {
        if (type === ButtonType.SPEND) setShowModal(ButtonType.SPEND);

        if (type === ButtonType.SAVE) router.push("/(app)/payment");

        if (type === ButtonType.SCAN) console.log(type);
    };
                ...
                {list.map((button) => (
                    <View style={styles.column} key={button.label}>
                        ...
                        >
                            ...
                            {labelPosition == "inside" && <Text style={[styles.labelText, { color: "#FFFFFF", fontSize: 16 }]}>{button.label}</Text>}
                        </TouchableOpacity>

                        {labelPosition == "outside" && <Text style={[styles.labelText, { color: "#2F2F2F", fontSize: 14 }]}>{button.label}</Text>}
                    </View>
                ))}
            </View>
        </>
