...
const TaskDelete = ({ taskId, shadow = true }: IProps) => {
    ...
    const handleDeleteTask = async () => {
        try {
            setLoading(true);
            if (deleteOption === "DELETE") await tasksService.deleteTask(taskId);

            if (deleteOption === "DELETE_OCCURENCE") await tasksService.updateTask(taskId, { disable: true });
        } catch (error: any) {
    ...
    return (
        <>
            <TouchableOpacity
                style={[styles.button, shadow && styles.buttonShadow, { backgroundColor: "#FD618C", shadowColor: "#D1325E" }]}
                onPress={() => {
                    setShowFrequencyModal(ModalSteps.OPTIONS);
                }}
            >
                <View>
            ...
            {showFrequencyModal === ModalSteps.OPTIONS && (
                <ModalComponent
                    visible={showFrequencyModal === ModalSteps.OPTIONS}
                    onClose={() => setShowFrequencyModal(ModalSteps.HIDE)}
                ...
                >
                    <View style={styles.modalContainer}>
                        ...
                        <Text style={styles.modalText}>
                            Cette tâche fait partie d'une récurrence. Souhaitez-vous supprimer uniquement cette occurrence ou toutes les répétitions à
                            venir ?
                        </Text>
                        <Pressable
                            style={[
                                deleteOption === "DELETE_OCCURENCE" ? styles.inputContainerSelected : styles.inputContainer,
                                { paddingVertical: 12 },
                            ]}
                            onPress={() => {
                                setDeleteOption("DELETE_OCCURENCE");
                            }}
                        >
                            <Text style={[styles.textInput, { fontWeight: deleteOption === "DELETE_OCCURENCE" ? "700" : "400" }]}>
                                Supprimer cette occurence uniquement
                            </Text>
                            <Ionicons
                                name={deleteOption === "DELETE_OCCURENCE" ? "ellipse" : "ellipse-outline"}
                                size={20}
                                color={deleteOption === "DELETE_OCCURENCE" ? "#846DED" : "#D5D5D5"}
                            />
                        </Pressable>
                        <Pressable
                        ...
                        >
                            <Text style={[styles.textInput, { fontWeight: deleteOption === "DELETE" ? "700" : "400" }]}>
                                Supprimer toute les répétitions à venir
                            </Text>
                            <Ionicons
                                name={deleteOption === "DELETE" ? "ellipse" : "ellipse-outline"}
                                size={20}
                                color={deleteOption === "DELETE" ? "#846DED" : "#D5D5D5"}
                            />
                        </Pressable>
                        <View style={{ display: "flex", flexDirection: "row", justifyContent: "space-between", alignItems: "center", width: "100%" }}>
                            <Pressable
                                style={[styles.button, { backgroundColor: "#D5D5D5", width: "47%" }]}
                                onPress={() => {
                                    setShowFrequencyModal(ModalSteps.CLOSE);
                                }}
                            >
                            ...
                            <Pressable
                                style={[styles.button, styles.buttonShadow, { backgroundColor: "#FD618C", shadowColor: "#D1325E", width: "47%" }]}
                                onPress={() => {
                                    setShowFrequencyModal(ModalSteps.CONFIRME);
                                }}
                            >
            ...
            {showFrequencyModal === ModalSteps.CONFIRME && (
                <ModalComponent
                    visible={showFrequencyModal === ModalSteps.CONFIRME}
                    onClose={() => setShowFrequencyModal(ModalSteps.HIDE)}
                    backgroundColor="#FFFFFF"
                    custom={true}
                >
                    <View style={styles.modalContainer}>
                        ...
                        <View style={{ display: "flex", flexDirection: "column", gap: 16, alignItems: "center", width: "100%" }}>
                            <Pressable
                                style={[styles.button, styles.buttonShadow, { backgroundColor: "#FD618C", shadowColor: "#D1325E", width: "100%" }]}
                                onPress={() => {
                                    handleDeleteTask();
                                }}
                            >
                            ...
                            <Pressable
                                style={[styles.button, { backgroundColor: "#D5D5D5", shadowColor: "#6A6A6A", width: "100%" }]}
                                onPress={() => {
                                    setShowFrequencyModal(ModalSteps.OPTIONS);
                                }}
                            >
            ...
            {showFrequencyModal === ModalSteps.CLOSE && (
                <ModalComponent
                    visible={showFrequencyModal === ModalSteps.CLOSE}
                    onClose={() => setShowFrequencyModal(ModalSteps.HIDE)}
                    backgroundColor="#FFFFFF"
                    custom={true}
                >
                    <View style={styles.modalContainer}>
                        ...
                        <Pressable
                            style={[styles.button, styles.buttonShadow, { backgroundColor: "#846DED", shadowColor: "#4E31CF", width: "100%" }]}
                            onPress={() => {
                                setShowFrequencyModal(ModalSteps.HIDE);
                                router.back();
                            }}
                        >
