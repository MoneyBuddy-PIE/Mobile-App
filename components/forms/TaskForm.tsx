...
const TaskForm = ({childId, type, task}: IProps) => {
    ...
    const [showCustomAmount, setShowCustomAmount] = useState(task?.moneyReward || false);

	const [selectedCoin, setSelectedCoin] = useState("");
	const [customCoin, setCustomCoin] = useState(task?.coinReward?.toString() ?? "");
	const [showCustomCoin, setShowCustomCoin] = useState(task?.coinReward || false);
    ...
    const handleCreateTask = async () => {
		...
		if (taskType === TaskType.WEEKLY)
			data.weeklyDays = weekdays

		if (taskType === TaskType.MONTHLY)
			data.monthlyDay = 1
        ...
        <View style={styles.section}>
			...
			<View style={styles.section}>
				...
				<View style={styles.amountContainer}>
					{PREDEFINED_COINS.map((coin) => (
						...
						>
							<Text
								style={[
									styles.amountButtonText,
									selectedCoin === coin && styles.amountButtonTextSelected,
								]}
							>
								{coin}
							</Text>
					...
					<TouchableOpacity
						style={[styles.amountButton, showCustomCoin && styles.amountButtonSelected]}
						onPress={handleCustomCoin}
					>
						<Text
							style={[styles.amountButtonText, showCustomCoin && styles.amountButtonTextSelected]}
						>
							Définir
						</Text>
					</TouchableOpacity>
		...
		{isUpdate && task?.id &&
            ...
            <View style={{display: "flex", flexDirection: "row-reverse", alignItems: "center", gap: 8}}>
                ...
                <TouchableOpacity
                    style={[
                        styles.button,
                        {backgroundColor: "#D5D5D5", shadowColor: "#979797"}
                    ]}
                    onPress={() => router.back()}
                >
	...
	return (
		<ModalComponent visible={visible} onClose={onClose} backgroundColor="#fff">
			<View>
					...
					radios.map((radio) => {
						...
						return (
							...
							>
								<Pressable 
								key={radio.value}
								onPress={() => onTaskTypeChange(radio.value)}
								>
									<View style={[isSelected ? styles.inputContainerSelected : styles.inputContainer, {paddingVertical: 12}]}>
										...
										<Ionicons 
											name={isSelected ? "ellipse" : "ellipse-outline" } size={20} color={isSelected ? "#846DED" : "#D5D5D5"}
										/>
									</View>
								</Pressable>
								{
										...
										<View style={{display: "flex", flexDirection: 'row', gap: 8, alignItems: "center"}}>
											{
												Object.values(WeekDay).map((day, index) => {
													...
													return (
														...
														>
															<Text style={[styles.weekdDayButtonText, isChecked && styles.weekdDayButtonTextSelected]}>{item.label.substring(0, 1)}</Text>
														</Pressable>
													)
												})
											}
				...
				<TouchableOpacity
					style={[styles.createButton, {marginHorizontal: 20}]}
					onPress={onClose}
				>
